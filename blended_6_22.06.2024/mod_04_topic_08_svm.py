...
data['LenOfDescription'] = data['Description'].str.len()


# %%
data['LenOfDescription'].isna().sum()

# %% 

data['LenOfDescription'] = data['LenOfDescription'].fillna(0).astype(int)
...
pet = pd.DataFrame(
    data={
        ...
        'PhotoAmt': 2
        #'LenOfDescription': 1500
    },
    index=[0])

# %%
pet['LenOfDescription'] = pet['Description'].str.len()
pet = pet.drop(columns='Description')
pet['Fee'] = pet['Fee'].astype(bool).astype(int).astype(str)
pet[num_cols] = kbins.transform(pet[num_cols]).astype(int).astype(str)

with warnings.catch_warnings():
    warnings.simplefilter('ignore')
    pet_enc = encoder.transform(pet)
    pet_transformed0 = scaler.transform(pet_enc)
    prob = clf.predict_proba(pet_transformed0).flatten()
